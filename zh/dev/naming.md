...
> [!TIP]
> 本规范适用于所有项目的标识符命名，涵盖文件、变量、常量、函数、类、组件、类型、接口等。语言/框架有强制约定时（如环境变量、特定文件名），优先遵循其约定。 <br/>
> **请知悉：本规范只为促进作用，并不强制执行。但团队成员 / Agents 在开发时请注意使自己的代码满足人类可读要求。**
...
- 避免使用拼音命名
- 避免使用*无意义*缩写 (`l` -> `logger` 之类可适当使用以简化代码)
- 避免使用单字母变量（`for` 循环的 `i`、`j`、`k` 除外）
- 避免在复杂情况下使用 `temp`、`data`、`info` 等无信息量的名称
- **禁止**在同一作用域使用仅大小写不同的两个标识符
