...
- [`.markdownlint.jsonc`](https://github.com/siiway/homepage/blob/main/.markdownlint.jsonc)：markdownlint 规则配置。
- [`.pymarkdown.yaml`](https://github.com/siiway/homepage/blob/main/.pymarkdown.yaml)：PyMarkdown（Python 版 markdownlint）等价规则配置。
- [`.prettierrc`](https://github.com/siiway/homepage/blob/main/.prettierrc)：Prettier 格式化配置。
...
### 12.2 Python 版：PyMarkdown

使用 Python 工具链的成员可改用 [PyMarkdown](https://github.com/jackdewinter/pymarkdown)（`pip install pymarkdownlnt` 或 `uv tool install pymarkdownlnt`）替代 Node 版 markdownlint，等价配置见仓库根目录的 [`.pymarkdown.yaml`](https://github.com/siiway/homepage/blob/main/.pymarkdown.yaml)，运行 `pymarkdown scan .` 时会自动加载。

> [!NOTE]
> PyMarkdown 与 markdownlint 规则基本一致，但受实现限制，**以下规则无法在 Python 版中校验**（仍由 Prettier 统一保证）：
>
> - **MD049**（斜体统一为 `_`）—— PyMarkdown 未实现该规则。
> - **MD050**（加粗统一为 `**`）—— PyMarkdown 未实现该规则。
>
> 此外 PyMarkdown 不读取 `.markdownlintignore`，需在命令中显式指定扫描路径来排除 `node_modules` 等目录；VitePress frontmatter 需由 `.pymarkdown.yaml` 中的 `front-matter` 扩展识别（已默认开启）。
